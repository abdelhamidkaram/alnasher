...
class _AddProductImagesState extends State<AddProductImages> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: AppSharedPreferences.isLogin(),
          ...
          return Center(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(
                      'يجب تسجيل حساب جديد او تسجيل الدخول من أجل نشر إعلاناتك '),
                  SizedBox(height: 20.h,),
                  CustomButton(
                    onTap: () {
                      goTo(path: AppRouteStrings.login,
                          context: context,
                          replacement: false,
                          args: NoArgs());
                    },
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        HomeCubit.get(context).getHomeResponse().then((value) {
          ...
          goTo(path: AppRouteStrings.home,
              context: context,
              replacement: false,
              args: NoArgs());
        });
